...
import com.sanri.app.wscall.exception.WsdlCallException;
...
public class WsdlOperation {
	private static Log logger = LogFactory.getLog(WsdlOperation.class);
	...
	public String invoke(String soapMessage) throws IOException{
		logger.info("调用 "+postMessageUrl+" 的:"+this.getOperation().getName()+" 方法时发送的　soap 消息为:\n"+soapMessage);
		String postSoapMessage = HttpUtil.postSoap(postMessageUrl, soapMessage, soapType, this.soapActionURI);
		logger.info("调用 "+postMessageUrl+" 的:"+this.getOperation().getName()+" 方法时响应的　soap 消息为:\n"+postSoapMessage);
		...
		case SOAP11:
			try {
				...
				if("document".equalsIgnoreCase(style)){
					...
					//先拼接方法名称
//					Element methodElement = DOMUtil.addElement(bodyNode, this.operation.getName());
...
//	}
	 ...
	 */
	void parserOutput() {
		logger.debug("解析方法:"+bindingOperation.getName()+" 出参");
		if(soapType == SOAPType.SOAP11 || soapType == SOAPType.SOAP12){
			Output output = operation.getOutput();
			Message message = output.getMessage();
			this.output = findWsdlParam(message);
			logger.info(this.name+" 得到出参为:"+this.output);
		}else if(soapType == SOAPType.HTTP_GET){
			logger.warn("暂时未实现 get 请求参数解析");
		}else if(soapType == SOAPType.HTTP_POST){
			logger.warn("暂时未实现 post 请求参数解析");
		}
	}
	 ...
	 */
	void parserInput() {
		logger.debug("解析方法:"+bindingOperation.getName()+" 入参");
		if(soapType == SOAPType.SOAP11 || soapType == SOAPType.SOAP12){
			Input input = operation.getInput();
			Message message = input.getMessage();
			this.input = findWsdlParam(message);
			logger.info(this.name+" 得到入参为:"+this.input);
		}else if(soapType == SOAPType.HTTP_GET){
			logger.warn("暂时未实现 get 请求参数解析");
		}else if(soapType == SOAPType.HTTP_POST){
			logger.warn("暂时未实现 post 请求参数解析");
